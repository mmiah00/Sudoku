    ...
    def findSolutions(self, nodeindex):
        if (self.done()):
            ...
            if nodeindex < len(self.open_boxes):
                cliques = self.possibleshelper (self.open_boxes[nodeindex].id) #find all the cliques that is related to the index
            ...
            for guess in possibles:
                if nodeindex < len(self.open_boxes):
                    self.open_boxes[nodeindex].data = guess #a guess
                    self.trials += 1
                    if (self.findSolutions(nodeindex + 1)): #worked, so try the next one
                        return True
...
#                     return False
#
#     def findSolutions(self, nodeindex, i):
#         if (self.done()):
#             return True
#         else:
#             nums = self.open_boxes[nodeindex].possibles
...
#                     if (self.findSolutions(nodeindex + 1, i + 1)):
#                         return True
#                     else:
#                         self.backtracks += 1
#                         self.open_boxes[nodeindex].data = '_'
